"""
SNTS — ORM Models
Tables: users, foods, food_logs, rewards, reward_redemptions, user_stats
"""
...
class User(db.Model, UserMixin):
    ...
    stats       = db.relationship('UserStats', backref='user', uselist=False,
                                  cascade='all, delete-orphan')
    redemptions = db.relationship('RewardRedemption', backref='user', lazy=True,
                                  cascade='all, delete-orphan')
...
class Reward(db.Model):
    ...
    file_name       = db.Column(db.String(255), nullable=True)   # original filename shown to user
    file_path       = db.Column(db.String(512), nullable=True)   # path on disk (relative to UPLOAD_FOLDER)
    ...
    @property
    def file_ext(self):
        """Return upper-case extension without dot, e.g. 'PDF'."""
        if self.file_name:
...
class RewardRedemption(db.Model):
    """
    Tracks every time a user redeems a reward.
    One user can redeem the same reward multiple times (each time
    deducting points), so this is intentionally not unique(user,reward).
    """
