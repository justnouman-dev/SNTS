...
def rewards():
    ...
    active_rewards = Reward.query.filter_by(is_active=True)\
        .order_by(Reward.points_required).all()

    # Build a set of reward_ids already redeemed by this user
    redeemed_ids = {
        r.reward_id
        for r in RewardRedemption.query.filter_by(user_id=current_user.id).all()
    }

    return render_template('user/rewards.html',
...
def redeem_reward(reward_id):
    ...
    if reward.has_file:
        flash(
            f'🎉 Reward redeemed: <strong>{reward.name}</strong>. '
            f'Your file is ready — click <strong>Download</strong> to get it!',
            'success'
        )
    else:
...
@user_bp.route('/download-reward/<int:reward_id>')
@login_required
def download_reward(reward_id):
    """
    Serve the reward file ONLY if:
      1. The reward exists, is active, and has a file.
      2. The current user has at least one redemption record for it.
    """
    reward = Reward.query.get_or_404(reward_id)

    # Gate: must have redeemed
    redemption = RewardRedemption.query.filter_by(
        user_id=current_user.id, reward_id=reward.id
    ).first()
    if not redemption:
        flash('You must redeem this reward before downloading it.', 'warning')
        return redirect(url_for('user.rewards'))

    if not reward.has_file:
        flash('This reward has no downloadable file.', 'info')
        return redirect(url_for('user.rewards'))

    upload_folder = current_app.config['UPLOAD_FOLDER']
    # Verify the file still exists on disk
    if not os.path.isfile(os.path.join(upload_folder, reward.file_path)):
        flash('File not found on server. Please contact the admin.', 'danger')
        return redirect(url_for('user.rewards'))
    ...
    return send_from_directory(
        ...
        reward.file_path,
        as_attachment=True,
        download_name=reward.file_name,   # present the original name to the browser
    )
